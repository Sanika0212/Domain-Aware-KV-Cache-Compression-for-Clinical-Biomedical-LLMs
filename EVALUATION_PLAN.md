...
## 7. Research Questions vs. Actual Measured Results

See `results/summary.csv` (generated by `benchmarks/plot_results.py`) for the
real numbers from the run described in §5.2. This section is intentionally
**not** pre-filled with target numbers — the previous version of this
document stated specific percentages (e.g. "50-70% memory reduction") before
any code existed to measure them, which is exactly the kind of unverified
claim this rebuild was meant to eliminate. Read the actual `results/` output
and the README's results table for what was actually measured, on what
hardware, at what scale.

---

## 8. Implementation Status

- [x] Real per-section budgeting on a real KV cache (`domain_kv.press.DomainAwarePress`).
- [x] Real public dataset loaders (PubMedQA, synthetic EHR-structured notes).
- [x] Real baseline comparison (`oracle`, `random`, `knorm`, `snapkv`) via real `kvpress` presses.
- [x] Real measured results in `results/` (small-model, CPU-only scale).
- [ ] Section-ablation sweep (Experiment 2, §5.2).
- [ ] Run on a 7-8B biomedical model on a GPU (architecture-compatible code is in place; not yet run due to no local GPU).
- [ ] MIMIC-licensed real-EHR benchmark (would require PhysioNet credentialing, out of scope for this public repo).

---

## 9. Evaluation Code

- `benchmarks/loader.py` — real dataset loaders (PubMedQA + synthetic).
- `benchmarks/runner.py` — real model, real compression, real generation, real scoring.
- `benchmarks/plot_results.py` — turns `results/runs.csv` into `results/summary.csv` + plots.
- `run_demo.py` — interactive single-note demo with real before/after numbers.
- `tests/test_*.py` — unit tests for section parsing, budget allocation, and `DomainAwarePress`; `tests/test_pipeline_smoke.py` is a real (slow, opt-in) end-to-end integration test.

**Quick start:**

```bash
PYTHONPATH=src:. python run_demo.py
PYTHONPATH=src:. pytest                       # fast unit tests
PYTHONPATH=src:. pytest -m slow               # + real model integration test
PYTHONPATH=src:. python benchmarks/runner.py --out results/runs.csv
PYTHONPATH=src:. python benchmarks/plot_results.py --csv results/runs.csv
```
...
**Key Claims:**
1. First work to apply per-section KV budgeting to clinical EHR document structure (implemented: `domain_kv.press.DomainAwarePress`).
2. First public benchmark of any KV-compression method (random/Knorm/SnapKV/ours) on a clinical/biomedical long-context task (PubMedQA + synthetic EHR-structured notes — see `results/`).
3. Memory/accuracy tradeoff is empirically measured, not asserted — see the README's results table for the actual numbers at the actual scale tested.
4. Engine-agnostic via real `kvpress` forward hooks (works on any `kvpress`-supported architecture: Llama/Mistral/Phi3/Qwen2/Qwen3/Gemma3; no vLLM dependency).
